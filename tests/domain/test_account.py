...
from app.domain.order import MarketOrder, LimitOrder, OrderType
...
def test_insufficient_funds(empty_account, sample_stock):
    ...
    order = MarketOrder(sample_stock, 10, OrderType.BUY) # Costs $1500, only have $1000
...
def test_successful_sell_order(empty_account, sample_stock):
    ...
    order = MarketOrder(sample_stock, 2, OrderType.SELL)
...
def test_pending_limit_order(empty_account, sample_stock):
    ...
    order = LimitOrder(sample_stock, 2, OrderType.BUY, limit_price=Decimal("140.00"))
